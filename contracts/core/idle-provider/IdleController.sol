...
contract IdleController is IController, IIdleCumulator, IYieldOraclelizable {
    ...
    // uniswap pairs for Gov tokens to underlying
    ...
    // uniswap cumulative prices needed for Gov tokens to underlying
    mapping(address=>uint256[]) public uniswapPriceCumulatives;
      ...
      public
    {
      ...
      ICToken cToken = ICToken(IdleProvider(pool).cToken());
      address[] rewardTokens = IdleProvider(pool).govTokens();
    ...
    returns (uint256 compGot, uint256 underlyingHarvestReward)
    {
        ...
        //ICToken cToken = ICToken(CompoundProvider(pool).cToken());
        IdleToken cToken = IdleToken(IdleProvider(pool).cToken());
        IERC20 uToken = IERC20(IdleProvider(pool).uToken());
        //IComptroller comptroller = IComptroller(cToken.comptroller()); //TODO
        //IERC20 rewardToken = IERC20(comptroller.getCompAddress());
        address[] memory rewardTokens = IERC20(IdleProvider(pool).getGovTokens());
      ...
      private
    {
      ...
      if (prevExchangeRateCurrent > 0) {
        // cumulate a new supplyRate delta: cumulativeSupplyRate += (cToken.exchangeRateCurrent() - prevExchangeRateCurrent) / prevExchangeRateCurrent
        ...
        if (prevCTokenBalance_ > 0) {
          ...
          cumulativeDistributionRate += poolShare.mul(EXP_SCALE).div(cTokensToUnderlying(prevCTokenBalance_, prevExchangeRateCurrent));
        }
      }
